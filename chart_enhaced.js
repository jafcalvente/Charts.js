...
(function(){
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		template = helpers.template = function(templateString, valuesObject, side){

			// If templateString is function rather than string-template - call the function for valuesObject

			if(templateString instanceof Function){
			 	return templateString(valuesObject, side);
		 	}
	...
	ChartEnhaced.Scale = ChartEnhaced.Element.extend({
		...
		buildYLabels : function(){
			...
			for (var i=0; i<=this.steps; i++){
				this.yLabels.push(template(this.templateString,{value:(this.min + (i * this.stepValue)).toFixed(stepDecimalPlaces)}, 'left'));
				this.yLabelsRight.push(template(this.templateString,{value:(this.min + (i * this.stepValue)).toFixed(stepDecimalPlaces)}, 'right'));
			}
			this.yLabelWidth = (this.display && this.showLabels) ? longestText(this.ctx,this.font,this.yLabels) + 10 : 0;
			this.yLabelRightWidth = (this.display && this.showLabels) ? longestText(this.ctx,this.font,this.yLabelsRight) + 10 : 0;
		},
		...
		fit: function(){
			...
			// El valor de 30 da un espacio extra que permite poner los valores de las barras en los diagramas sin cortarse
			this.startPoint += this.padding + 30;
		...
		calculateXLabelRotation : function(){
			...
			this.xScalePaddingRight = (lastWidth/2 + 3 > this.yLabelRightWidth) ? lastWidth/2 + 3 : this.yLabelRightWidth;
			...
			if (this.display){
				...
				each(this.yLabels,function(labelString,index){
					var yLabelCenter = this.endPoint - (yLabelGap * index),
						...
						showAxis = this.showYAxis,
						showSecondYAxis = this.showSecondYAxis,
						yLabelRightWidth = this.yLabelRightWidth;
					...
					if (this.showLabels){
						...
						if (showSecondYAxis) {
							ctx.textAlign = "left";
							ctx.fillText(this.yLabelsRight[index], xEnd + 2, yLabelCenter);
							ctx.textAlign = "right";
						}
					}
				...
				each(this.xLabels,function(label,index){
					...
					if (index === 0 && !drawVerticalLine && showAxis) {
					...
					// Dibujar el eje Y al final
					if (showAxis && showSecondYAxis && index === this.xLabels.length - 1) {
						ctx.beginPath();
						ctx.lineWidth = this.lineWidth;
						ctx.strokeStyle = this.lineColor;
						ctx.moveTo(xEnd - 5, this.endPoint);
						ctx.lineTo(xEnd - 5, this.startPoint - 3);
						ctx.stroke();
						ctx.closePath();
					}
	...
	ChartEnhaced.Type.extend({
		...
		buildScale : function(labels){
			...
			var scaleOptions = {
				...
				showYAxis : this.options.showYAxis,
				showSecondYAxis : this.options.showSecondYAxis,
